...
public class MapDisplay extends JPanel{
	...
	private void Initial() {
		...
		img = Toolkit.getDefaultToolkit().getImage("campus.jpg");
		building_label.setText("campus");
		repaint();
		//System.out.println("change image to campus");
		for(Point p:array) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(qpanel.point != null) {
					...
					for(Point p:pointarray) {
						if(!p.getFeature().equals("null")) {
							LocationLabel l = new LocationLabel(p.getRoomNum(), p.getxCooridinate(), p.getyCooridinate());
							l.setPoint(p);
							add(l);
